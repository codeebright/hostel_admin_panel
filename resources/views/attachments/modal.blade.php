                ...
                @foreach($attachments as $item)
                  <tr id="attachment_{{$item->id}}">
                    ...
                    <td>
                       ...
                       <a class="text-decoration-none" href="#" onclick="serverRequest('{{route('attachment.edit')}}','id={{encrypt($item->id)}}&&room_id={{encrypt($item->record_id)}}&&hostel_id={{encrypt($item->parent_id)}}&&table={{encrypt($table)}}&&file_name={{$item->filename}}','POST','attachment-div')"><i class="fa fa-edit text-dark"></i></a> |
